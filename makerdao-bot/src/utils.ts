...
export const getBalanceAndTotalSupply = async (
  ...
  const L1DAI = new ethers.Contract(l1Dai, IDAI, l1Provider);
  const L2DAI = new ethers.Contract(l2Dai, IDAI, l2Provider);
  ...
  if (cache.has(l1CacheKey)) {
    ...
    l1Balance = await L1DAI.balanceOf(l1Escrow);
  ...
  if (cache.has(l2CacheKey)) {
    ...
    l2TotalSupply = await L2DAI.totalSupply();
